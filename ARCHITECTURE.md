...
| Markdown source layer | `markdown/MarkdownSource.kt`, `markdown/MarkdownSourceRange.kt` | `MarkdownSource` (internal line map: `\n`/`\r\n`/`\r`, BOM, final-newline state, verbatim `slice(...)`), `MarkdownLineEndings` (internal), `MarkdownSourceRange` (public half-open UTF-16 range), `MarkdownSourceLocation`, `MarkdownSourceLocator` (public lookup, internal constructor) |
...
| Markdown fidelity report | `markdown/MarkdownFidelityReport.kt` | `MarkdownFidelityReport`, `MarkdownEditModeRecommendation` (`Native`/`RawFallback`), `MarkdownFidelityAnalyzer` (internal; one decode + one reused encode; impact-driven `nativeEditingSafe`, `wouldRewriteSource`) |
...
| Markdown HTML bridge | `markdown/MarkdownHtmlBridge.kt` | `MarkdownHtmlBridge` (internal; inline paired-tag + block HTML detection, routing through `HtmlFragmentDecoder` with inner text handed back to the Markdown inline parser), `MarkdownHtmlSpanEncoders` (`Underline`→`<u>`, `Highlight`→`<mark data-cascade-highlight>`), `MARKDOWN_PRESERVED_HTML_TYPE_ID` |
| Markdown profile config | `markdown/MarkdownProfile.kt`, `markdown/MarkdownProfileSupportSet.kt` | `MarkdownProfile` / `.Default` (immutable encoder registrations, policy slots, `withSupportSet`, strict `withoutHtmlBridge()` composite), `MarkdownProfileSupportSet` (block/span/document predicates; the finalized default claim verifies documents through a clean encode; `.None` placeholder). Decode grammar and emphasis dialect injection are intentionally not public; the unreleased extension surface was retired when grammar ownership moved to JetBrains Markdown. |
...
| Markdown parse projection + state | `markdown/MarkdownAstDecoder.kt`, `markdown/MarkdownDecodeState.kt` | `MarkdownParseInput` strips BOM and normalizes CRLF/CR to LF for JetBrains while mapping every boundary back to original UTF-16; `MarkdownParseState` owns bounded warnings, definitions, block/span counts, and fatal abort state |
...
| Markdown entity table | `markdown/MarkdownEntities.kt` | `MarkdownEntities` (internal; full numeric references with the U+FFFD invalid-codepoint rule, documented named subset shared with `HtmlEntityDecoder`, unknown names literal + `UnsupportedEntity` warning at the call site), `MarkdownEntityMatch` |
...
**LinkUrlPolicy** — two link URL contracts split by call-site intent. `validate(...)` is user-entry normalization for link creation chrome (toolbar/popup): anything non-blank is accepted, inputs without a `<scheme>://` prefix get `https://` prepended. `validateStoredTarget(...)` is stored-target validation for codec/persistence paths (`RichTextSchema` JSON, HTML `<a href>` decode, Markdown link decode): trim + nonblank only, the target string is otherwise preserved exactly so relative/fragment/`mailto:`/`tel:`/custom-scheme targets round-trip byte-identically. The only `LinkValidationError` in both contracts is `Blank`. Platform URL opening, rendering, mutation actions, and allowlists are separate feature layers.
...
| `HtmlLinkAttributesTest.kt` | Default `<a href>` stored-target preservation (relative/fragment/mailto/tel/custom-scheme hrefs kept exactly), missing/blank href warning/drop behavior, and `<mark>` highlight color decode |
| `htmlserialization/HtmlFragmentDecoderTest.kt` | Internal fragment entry points: inline `<u>`/`<em>` span decode, block fragment quote/paragraph decode, root inline runs kept, text-leaf reparser hook plumbing (invocation, span contribution, throw degradation), empty fragments |
| `markdown/MarkdownSourceTest.kt` | Source layer: CRLF/LF/CR/mixed line maps, BOM recording and location lookup past it, final-newline state, verbatim slicing across line boundaries, first/last/empty-line and clamped location lookups |
...
| `markdown/MarkdownHtmlBridgeTest.kt` | HTML bridge: `<u>`/`<mark>` decode+encode round-trip, inner-warning DataLoss + analyze RawFallback, empty-island preservation, unmatched-flood termination, partial overlap island |
| `markdown/MarkdownStrictProfileTest.kt` | `withoutHtmlBridge()`: inline HTML stripped with DataLoss, analyze RawFallback, no raw HTML through any emission path (spans/verbatim/preservedHtml/`<script>`), no Underline/Highlight claim |
| `markdown/MarkdownCorpusTest.kt` + `MarkdownCorpusFixtures.kt` | Data-driven corpus manifest: expected `analyze` recommendation + warning-impact categories per fixture, Native fixtures round-trip |
| `markdown/MarkdownReferenceCorpusTest.kt` + `MarkdownReferenceProfiles.kt` | StrictGfm/StrictHardBreak reference field profiles, #339 blank-line and #348 numbered-continuation classes, dialect round-trip |
| `markdown/MarkdownDifferentialTest.kt` | Selected CommonMark 0.30 / GFM examples by number: byte-identical round-trip (Aligned) or documented Deviation, no third state |
| `markdown/MarkdownFuzzTest.kt` | Random + multi-KB + deep-nesting input across retained reference profiles: terminates, no-throw, bounded warnings/output |
| `markdown/MarkdownPerformanceBaselineTest.kt` (desktopTest) | 64 KB field regression guard: decode/encode ≤ 250 ms, analyze ≤ 750 ms, adversarial ≤ 1 s (post-integration local median decode 16 / encode 14 / analyze 28 ms) |
| `screens/markdownfield/MarkdownFieldScreenModelTest.kt` (sample) | Markdown field host contract: unsupported→raw+0 writes, external-revision→Conflict (edit retained), matching-revision→exact canonical persisted once, no-edit→no write, lossy edit→RejectedLossy+0 writes |
