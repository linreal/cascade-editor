...
class HtmlSupportSetTest {
    ...
    fun `supportsSpan accepts Link with any url`() {
        ...
        assertTrue(supportSet.supportsSpan(SpanStyle.Link("https://example.com/path?q=1")))
    }

    @Test
    fun `supportsSpan rejects Custom`() {
        assertFalse(supportSet.supportsSpan(SpanStyle.Custom(typeId = "custom.mention")))
    }
    ...
    fun `supportsDocument rejects a document containing an unsupported span style`() {
        val customSpan = TextSpan(0, 1, SpanStyle.Custom(typeId = "custom.mention"))
        val blocks = listOf(
